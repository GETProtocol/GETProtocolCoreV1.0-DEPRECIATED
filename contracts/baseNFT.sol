...
contract baseNFT is Initializable, ContextUpgradeable, ERC721BurnableUpgradeable {
    ...
    // function initialize_base(address _address_gAC, address _address_metadata, address _get_economics) public virtual initializer {
    ...
    // uint256 public constant mintGETfee = 1000000000000000000; // GET sent to collector 
    // uint256 public constant transferGETfee = 100000000000000000;
    // uint256 public constant eventGETfee = 100000000000000000;
    // uint256 public constant claimNFTfee = 100000000000000000;
    ...
    function secondaryTransfer(address originAddress, address destinationAddress, uint256 resalePrice) public {
        ...
        require(_nftInfo[nftIndex].valid == false, "GET TX FAILED Func: secondaryTransfer - getNFT is marked as invalidated");
    ...
    function scanNFT(address originAddress) public {
        ...
        require(_nftInfo[nftIndex].valid == false, "GET TX FAILED Func: scanNFT - getNFT is marked as invalidated");

        if (_nftInfo[nftIndex].valid == true) {
    ...
    function invalidateAddressNFT(address originAddress) public {
        ...
        require(gAC.hasRole(MINTER_ROLE, _msgSender()), "invalidateAddressNFT: sender must have minter role to mint");

        uint256 nftIndex = tokenOfOwnerByIndex(originAddress, 0);

        require(_nftInfo[nftIndex].valid != true, "GET TX FAILED Func: invalidateAddressNFT - getNFT is already set to true.");
        _nftInfo[nftIndex].valid = true;
    ...
    function claimgetNFT(address originAddress, address externalAddress) public {
        ...
        require(_nftInfo[nftIndex].valid == false, "GET TX FAILED Func: claimgetNFT - getNFT is marked as invalidated.");
        require(_nftInfo[nftIndex].scanned == true, "GET TX FAILED Func: claimgetNFT - getNFT is not scanned yet.");
    ...
    // function moveFuelToken(uint256 amount, IERC20 targetToken, address collectorAddress) public {
    //     uint256 _balance = targetToken.balanceOf(address(this));
    //     require(_balance > amount, "getNFT factory has too little GET - Please top-up GET to continue use.");
    //     targetToken.transfer(collectorAddress, amount);
    // }
