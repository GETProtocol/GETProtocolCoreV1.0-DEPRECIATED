...
contract GETProtocolConfigurationV2 is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    address public getNFT_ERC1155_proxy_address;
    ...
    event UpdateERC1155(address _old, address _new);
    ...
    function setAllContractsStorageProxies(
        ...
        getNFT_ERC1155_proxy_address = _erc1155_proxy;
    ...
    function setAccessControlGETProxy(address _access_control_proxy) external onlyOwner {
        require(isContract(_access_control_proxy), "_access_control_proxy not a contract");

        emit UpdateAccessControl(AccessControlGET_proxy_address, _access_control_proxy);
    ...
    function setBASEProxy(address _base_proxy) external onlyOwner {
        require(isContract(_base_proxy), "_base_proxy not a contract");

        emit UpdatebaseGETNFT(baseGETNFT_proxy_address, _base_proxy);
    ...
    function setERC1155Proxy(address _erc1155_proxy) external onlyOwner {
        require(isContract(_erc1155_proxy), "_erc1155_proxy not a contract");

        emit UpdateERC1155(getNFT_ERC1155_proxy_address, _erc1155_proxy);
    ...
    function setMetaProxy(address _metadata_proxy) external onlyOwner {
        require(isContract(_metadata_proxy), "_metadata_proxy not a contract");

        emit UpdateMetdata(eventMetadataStorage_proxy_address, _metadata_proxy);
    ...
    function setFinancingProxy(address _financing_proxy) external onlyOwner {
        require(isContract(_financing_proxy), "_financing_proxy not a contract");

        emit UpdateFinancing(getEventFinancing_proxy_address, _financing_proxy);
    ...
    function setPriceOracle(address _price_oracle_new) external onlyOwner {
        require(isContract(_price_oracle_new), "new _price_oracle is not a contract");

        emit UpdatePriceOracle(priceOracleAddress, _price_oracle_new);
    ...
    function setEconomicsProxy(address _economics_proxy) external onlyOwner {
        require(isContract(_economics_proxy), "_economics_proxy not a contract");

        emit UpdateEconomics(economicsGET_proxy_address, _economics_proxy);
    ...
    function setgetNFT_ERC1155(address _getNFT_ERC1155) external onlyOwner {
        require(isContract(_getNFT_ERC1155), "_getNFT_ERC1155 not a contract");

        emit UpdateERC1155(getNFT_ERC1155_proxy_address, _getNFT_ERC1155);

        getNFT_ERC1155_proxy_address = _getNFT_ERC1155;
    ...
    function _callSync() internal {
        // UPDATE BASE
        require(
            IsyncConfiguration(baseGETNFT_proxy_address).syncConfiguration(),
            "FAILED_UPDATE_BASE"
        );

        // UPDATE ECONOMICS
        require(
            IsyncConfiguration(economicsGET_proxy_address).syncConfiguration(),
            "FAILED_UPDATE_ECONOMICS"
        );

        // UPDATE METADATA
        require(
            IsyncConfiguration(eventMetadataStorage_proxy_address).syncConfiguration(),
            "FAILED_UPDATE_METADATA"
        );

        // UPDATE FINANCING
        require(
            IsyncConfiguration(getEventFinancing_proxy_address).syncConfiguration(),
            "FAILED_UPDATE_FINANCE"
        );
    }
    ...
    function setGETUSDPrice(uint256 _newGETUSDPrice) external {
    ...
    function setBufferAddressGlobal(address payable _newBufferGlobal) external onlyOwner {
    ...
    function setTreasuryDAO(address payable _newTreasury) external onlyOwner {
    ...
    function setStakingContract(address payable _newStaking) external onlyOwner {
    ...
    function setBasicTaxRate(uint256 _basicTaxRate) external onlyOwner {
     ...
     */
    function setGETUSD(uint256 _newGETUSD) external onlyOwner {
