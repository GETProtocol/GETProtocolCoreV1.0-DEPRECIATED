...
contract wrapGETNFT is ERC20, ReentrancyGuard {
    ...
    string constant public name = "Wrapped getNFT - GUTS Tickets";
    string constant public symbol = "wgetNFT - GUTS";
    ...
    address public getNFTCoreBase;
    IGETBase public baseNFT;
    ...
    function depositNFTAndMintTokens(address eventAddress, uint256 amountNFTs, uint256 collaterizationPrice) external nonReentrant {

        // TODO add modifier relayer/contract call

        require(amountNFTs > 0, 'AMOUNT CANNOT BE 0');

        // for(uint i = 0; i < _kittyIds.length; i++){
        for(uint i = 0; i < amountNFTs; i++){

            // uint256 kittyToDeposit = _kittyIds[i];
            uint256 nftToDeposit = baseNFT.tokenOfOwnerByIndex(eventAddress, 0);
            
            // require(msg.sender == kittyCore.ownerOf(kittyToDeposit), 'you do not own this cat');
            // require(eventAddress == baseNFT.ownerOf(nftToDeposit), 'DEPOSIT AND MINT');

            // require(kittyCore._tokenApprovals(kittyToDeposit) == address(this), 'you must approve() this contract to give it permission to withdraw this cat before you can deposit a cat');

            // kittyCore.transferFrom(msg.sender, address(this), kittyToDeposit);
            baseNFT.relayerTransferFrom(eventAddress, address(this), nftToDeposit);

            // _pushKitty(kittyToDeposit);
            _pushKitty(nftToDeposit);

            // emit DepositKittyAndMintToken(kittyToDeposit);
            emit DepositKittyAndMintToken(nftToDeposit);

        }
        _mint(msg.sender, (amountNFTs).mul(collaterizationPrice**18));
    }
    ...
    constructor(address getNFTBase_contract) public {
        baseNFT = IGETBase(getNFTBase_contract);
        // kittyCore = KittyCore(getNFTBase_contract);
    }
